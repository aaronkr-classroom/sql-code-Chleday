...
3. 하둡 (HDFS) 시스템은 하둡 프레임워크를 이루고 있는 다양한 서브 프로젝트들의 모임이다.
4. 저가의 서버라도 서버 자체를 더 추가한 뒤 서버들을 묶어 분산 병렬 처리함으로써 전체 시스템의 용량을 키우는 방식을 수평적 (horizontal) 또는 스케일 (수평) 확장 방식아라고 한다.
5. (데이터 마이닝)은 데이터 집합으로부터 의미있는 통계적 규칙이나 패턴을 찾아냄으로써 데이터의 새로운 가치를 찾아내는 기법이다.
6. (정보 그래픽)은 정보(information)와 그래픽(graphic) 의 합성어로 분석 결과를 쉽게 어해할 수 있도록 그래픽과 텍스트를 조합하여 시각적으로 표현하는 방법이다.

**18 SQL 데이터베이스와 NoSQL 데이터베이스의 특성과 차이점을 비교하시오.**
SQL 데이터베이스는 구조화된 데이터를 저장하고, 관계형 모델을 기반으로 쿼리 언어인 SQL을 사용하여 데이터를 처리합니다. ACID 속성(Atomicity, Consistency, Isolation, Durability)을 보장하며, 데이터 무결성과 일관성 유지에 중점을 둡니다. 예: MySQL, PostgreSQL
NoSQL 데이터베이스는 비정형 데이터와 대규모 데이터를 처리하는 데 적합하며, 관계형 데이터 모델이 아닌 다른 데이터 모델을 사용합니다. 확장성, 유연성, 속도가 강점이며, 주로 분산 시스템에서 높은 성능을 발휘합니다. 예: MongoDB, Cassandra, Redis
