...
3. 오픈 소스 DBMS이다.
4. 관계형 데이터베이스와 동일한 질의 기능을 지원한다. O
...
4. 스키마를 변경하거나 데이블을 추가하는 등 대웅이 복잡하고 어렵다. O

**05 몽고OB의 '_id' 필드에 관한 설명으로 옳은 것은?**

1. 저장된 모든 문서는 '_id' 필드를 가진다. O
...
3. 몽고DB (몽고디비 )은 문서에 대한 질의뿐만 아니라 관리나 실행중인 인스턴스를 점검하는데 유용한 도구로써 자바스크립트 해석 기능을 가지고 있어 자바스크립트 프로그램 작성과 실행이 가능하다.
...
3. db.회원.find();
4. db.회원.insertOne({_id: "m111", 나이: 55, 성별: "남"});
...
문서형(Document-based): JSON, BSON 기반으로 데이터를 저장하며, 유연하고 계층적 구조를 지원합니다. 예: MongoDB.
키-값(Key-Value): 단순한 키-값 쌍으로 데이터를 저장하며, 빠른 읽기와 쓰기가 장점입니다. 예: Redis, DynamoDB.
컬럼형(Column-family): 데이터가 컬럼 기반으로 저장되며 대규모 데이터 분석에 적합합니다. 예: Cassandra, HBase.
그래프형(Graph): 데이터 간의 관계를 저장하고 탐색하는 데 최적화되어 있습니다. 예: Neo4j.

**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**
스키마:

몽고DB: 동적 스키마(schema-less)로 유연성이 높음.
관계형 DB: 고정 스키마로 구조화된 데이터를 저장.
데이터 모델:

몽고DB: JSON 기반 문서 형식.
관계형 DB: 테이블 기반 데이터 모델.
확장성:

몽고DB: 수평적 확장(Sharding)을 기본적으로 지원.
관계형 DB: 주로 수직적 확장(Scale-up).
질의 언어:
