...
DESC 과목2; --MYSQL에서만 가능
SHOW CREATE TABLE 과목2; -- MYSQL에서만 가
...
CREATE USER manager WITH PASSWORD '1234'; -- 계정생성
GRANT ALL PRIVILEGES ON DATABASE univdb TO manager; -- 계정에 권한부여

ALTER DATABASE univdb OWNER TO manager; -- postgres 계정만 실행가능
-- 위 드랍다운 화살표 누르고 NEW Connection... 선택
-- univdb와 manager 계정 선택하고 저장하기
SELECT current_user; -- manager (새로운 사용자) 중간고사때 사용
