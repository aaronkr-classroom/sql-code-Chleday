...
검색 성능을 향상시켜 데이터를 빠르게 찾을 수 있다.
데이터 정렬 속도를 높여 성능 개선에 도움을 준다.
단점:

인덱스 생성과 유지에 추가적인 저장 공간이 필요하다.
데이터 수정 시 인덱스도 갱신되어야 하므로 성능에 영향을 줄 수 있다.
**18 인덱스를 사용하는 것이 바람직한 경우와 그렇지 않은 경우를 구분하여 설명하시오.**
인덱스를 사용하는 것이 바람직한 경우:
자주 조회되는 컬럼에 인덱스를 설정하면 성능 향상에 도움이 된다.
검색 조건에 자주 사용되는 컬럼에 인덱스를 설정할 때 유리하다.
인덱스를 사용하는 것이 바람직하지 않은 경우:
데이터의 수정이 자주 발생하는 경우에는 인덱스의 유지 비용이 크므로 사용을 자제해야 한다.
데이터 양이 적어서 인덱스의 성능 개선 효과가 미미할 때는 불필요하다.
